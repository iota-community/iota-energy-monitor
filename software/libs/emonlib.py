...
class Emonlib(object):
    ...
    async def voltage(self, _in_pin_voltage: int, _voltage_cal: float,
                      _phase_cal: float):
        """
    ...
    async def current(self, _in_pin_current, _current_cal: float):
        ...
        self._currentCalibration = 111.1
    ...
    async def calc_voltage_current(self, crossings: int, timeout: int):
        ...
        while cross_count < crossings and time.time() - start < timeout:
            ...
            self._offsetVoltage = self._offsetVoltage + (
                (sample_voltage - self._offsetVoltage) / 1024)
            self._filteredVoltage = sample_voltage - self._offsetVoltage
            self._offsetCurrent = self._offsetCurrent + (
                (sample_current - self._offsetCurrent) / 1024)
        ...
        voltage_ratio = self._voltageCalibration * (
            (SUPPLY_VOLTAGE / 1000) / ADC_COUNTS)
        self.voltageRms = round(
            voltage_ratio * sqrt(self._sumVoltage / number_samples), 3)

        current_ratio = self._currentCalibration * (
            (SUPPLY_VOLTAGE / 1000) / ADC_COUNTS)
        self.currentRms = round(
            current_ratio * sqrt(self._sumCurrent / number_samples), 3)

        self.realPower = round(
            voltage_ratio * current_ratio * self._sumPower / number_samples, 3)
    ...
    def calc_current_rms(self, _number_samples: int):
        """
        Calculate current rms
        :param _number_samples:
        :return:
        """
        adc_current = adc.channel(self._inPinI)

        for sample in range(0, _number_samples):
            sample_current = adc_current.value()

            self._offsetCurrent += ((sample_current - self._offsetCurrent) /
                                    1024)
            self._filteredCurrent = sample_current - self._offsetCurrent
        ...
        current_ratio = 105 * ((SUPPLY_VOLTAGE / 1000) / ADC_COUNTS)

        self.currentRms = current_ratio * sqrt(
            self._sumCurrent / _number_samples)
        self._sumCurrent = 0
